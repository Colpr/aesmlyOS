...
.more_select:
	pusha
	mov bl, 01110000b		; Black on white for option list box
	mov dl, 21
	mov dh, 6
	mov si, 38
	mov di, 22
...
.draw_white_bar:
	...
	mov bh, 0
	mov cx, 36
	mov bl,  	10000000b		; Black text on white background
	mov al, ' '
	int 10h
...
.one_button:
	mov bl, 00000111b		; Black on white
	mov dh, 14
	mov dl, 35
	mov si, 8
	mov di, 15
...
.two_button_wait:
	...
	mov bl, 10001111b		; White on red for cancel button
	mov dh, 14
	mov dl, 42
	mov si, 9
	mov di, 15
